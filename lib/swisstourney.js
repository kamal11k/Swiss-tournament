...
function registerNewPlayer(user_id,t_id,name, cb) {
	...
	connection.query(query, [user_id,t_id,name] ,function (error, results, fields) {
		...
		if (error) {
			...
			var query = `
			select name from player where user_id=? and tournament_id not in(?)
			`
			connection.query(query,[user_id,t_id], function(error,data){
				connection.end();
				if (error) {
					//console.log('nullnullnullnullnullnullnullnullnullnullnullnullnullnull')
					cb(error, 0);
		...
		if(error){
			...
			connection.query(query, [player_id,user_id,t_id,name] ,function (error, results, fields) {
				...
				if (error) {
					...
					var query = `
					select distinct name from player where user_id=? and tournament_id not in(?)
					`
					connection.query(query,[user_id,t_id], function(error,data){
						connection.end();
						if (error) {
...
function existingPlayers(user_id,t_id,cb){
	...
	var query = `
			select distinct name from player where user_id=? and tournament_id not in (?)
			`
	connection.query(query,[user_id,t_id], function(error,results){
		connection.end();
		if (error) {
			cb(error, 0);
		}
...
function viewTournament(user_id,cb) {
	var connection = get_connection();
	var query = 'select  id,name from tournament where user_id=?';
...
function getNextPair(playerStandings, tournament) {
	...
	res.end("Tournament finished.Cann't execute more rounds");
